...
public class MathLexer {
	...
	private TokenType readTokenType() throws LexerException {
	  TokenType type;
		switch (current) {
	  case '=': type = TokenType.EQUAL_SIGN; break;
	  case '(': type = TokenType.OPEN_PARENTHESIS; break;
	  case ')': type = TokenType.CLOSE_PARENTHESIS; break;
	  case '+': type = TokenType.PLUS_SIGN; break;
	  case '-': type = TokenType.HYPHEN_SIGN; break;
	  case '*': type = TokenType.ASTERISK_SIGN; break;
	  case '/': type = TokenType.SLASH_SIGN; break;
	  case '%': type = TokenType.PERCENTAGE_SIGN; break;
	  default: throw new LexerException("Unsupported symbol " + current);
	  }
		nextChar();
		return type;
  }
	...
	private void skipBlanks() throws LexerException {
		while (!endOfStream && current <= ' ') { nextChar(); }
	}
